...
class GameController:
    ...
    def reset_game(self):
        """Calls the ROS service to reset the game and robot position."""
        self.game_counter += 1
        position_to_request = self.game_counter % 2 # Alternates between 0 and 1 for start position
        ...
        try:
            ...
            rospy.signal_shutdown("Reset service call failed. Shutting down.") # Shut down if reset fails
    ...
    def run(self):
        ...
        while not rospy.is_shutdown() and self.game_counter < self.total_games:
            if not self.game_active:
                ...
                rospy.sleep(0.1) # 2 seconds pause between games
