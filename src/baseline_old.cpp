	...
	if (not this->pause){
		if (this->reset){
			if (ros::Time::now().toSec() - this->timestart > 3){
				if(this->start_pos.empty()){
					// int rand_choice = rand() % 2;
					int deterministic_choice = this->start_pos_toggle % 2;
					this->start_pos = (deterministic_choice == 0) ? this->start_pos_0 : this->start_pos_1;
					this->start_pos_toggle ++;
				}
