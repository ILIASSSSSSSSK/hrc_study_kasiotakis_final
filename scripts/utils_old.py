...
def get_SAC_agent(observation_space, chkpt_dir=""):
	...
	if chkpt_dir == "" or initialized_agent:
		...
		save_chkpt_dir = os.path.join(rospy.get_param("rl_control/Game/full_path", "tmp"), 'Paper_Results/rl_models/' + save_chkpt_dir)	
		i=1
...
def get_save_dir(load_model_for_training=False):
	full_path = os.path.join(rospy.get_param('rl_control/Game/full_path', '/home/ttsitos/catkin_ws/src/transfer_learning_SAC/'), 'Paper_Results/games_info')
...
def save_data(game, data_dir):
	with open(data_dir+'/data.csv', 'ab') as outputFile:
		headers = [['block'], ['episode'], ['expert'],['Rewards'], ['Episodes Duration in Seconds'], ['Travelled Distance'], ['Episodes Duration in Timesteps']]		
		np.savetxt(outputFile, list(zip(*headers)), delimiter=',', fmt='%s')
		data = [game.data_block_history, game.episode_history, len(game.reward_history) * [game.expert_action_flag],game.reward_history, game.episode_duration, game.travelled_distance, game.number_of_timesteps]		
		np.savetxt(outputFile, list(zip(*data)), delimiter=',')

	with open(data_dir+'/test_data.csv', 'ab') as outputFile:
		headers = [['block'], ['episode'], ['Rewards'], ['Episodes Duration in Seconds'], ['Travelled Distance'], ['Episodes Duration in Timesteps']]
		np.savetxt(outputFile, list(zip(*headers)), delimiter=',', fmt='%s')
		data = [game.test_data_block_history, game.test_episode_history, game.test_reward_history, game.test_episode_duration, game.test_travelled_distance, game.test_number_of_timesteps]
		np.savetxt(outputFile, list(zip(*data)), delimiter=',')
...
def save_train_data(game, i_block, data_dir):
	with open(data_dir+'/data_block_' + str(i_block) + '.csv', 'ab') as outputFile:
		headers = [['block'], ['episode'], ['expert'],['Rewards'], ['Episodes Duration in Seconds'], ['Travelled Distance'], ['Episodes Duration in Timesteps']]		
		np.savetxt(outputFile, list(zip(*headers)), delimiter=',', fmt='%s')
		data = [game.data_block_history, game.episode_history, len(game.reward_history) * [game.expert_action_flag],game.reward_history, game.episode_duration, game.travelled_distance, game.number_of_timesteps]		
		np.savetxt(outputFile, list(zip(*data)), delimiter=',')

	with open(data_dir+'/rl_data_block_' + str(i_block) + '.csv', 'ab') as outputFile:
		np.savetxt(outputFile, [game.state_info[0]], delimiter=',', fmt='%s')
		np.savetxt(outputFile, game.state_info[1:], delimiter=',')
