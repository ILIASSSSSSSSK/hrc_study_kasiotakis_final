...
#extra libraies used by christos
import torch
...
class RL_Control:
	def __init__(self):
		...
		self.min_a_x=-0.2
		self.min_a_y=-0.02
		#in the new edition of christos this does not exist 
		"""
		...
		#in the new edition of christos those have been added
		...
		if self.train_model:
			...
			if self.load_model_for_training:
				#self.load_model_for_training_dir = rospy.get_param("rl_control/Game/load_model_training_dir", "dir") #in the new edition it is just transfered above
				...
				if self.initialized_agent:
					...
					"""
						#self.initialized_agent_dir = rospy.get_param("/rl_control/Game/initialized_agent_dir","dir") #in the new edition it is just transfered above
					self.agent = get_SAC_agent(observation_space=[self.num_of_states])
					self.agent.load_baseline_models(self.initialized_agent_dir)#self.agent.load_models() #it is replaced
			...
			if self.transfer_learning:
				#he just changed the name of the variable and the variable is declared above  
			...
			self.load_model_for_testing_dir = rospy.get_param("rl_control/Game/load_model_testing_dir", "dir") #above
		...
		#self.episode_number=0 it does not exist
		...
		if self.num_of_states==4:
		 self.column_names = ("timestamps", "block", "episode", "init_pos", "human_action", "agent_action", "prev_x", "prev_y", "next_x", "next_y", "ee_pos_x_prev", "ee_pos_y_prev", "ee_vel_x_prev","ee_vel_y_prev","ee_pos_x_next", "ee_pos_y_next", "ee_vel_x_next","ee_vel_y_next","cmd_acc_human", "cmd_acc_agent")#self.column_names = ("human_action", "agent_action", "ee_pos_x_prev", "ee_pos_y_prev", "ee_vel_x_prev","ee_vel_y_prev","ee_pos_x_next", "ee_pos_y_next", "ee_vel_x_next","ee_vel_y_next","cmd_acc_human", "cmd_acc_agent")
		elif self.num_of_states==2:
		 self.column_names = ("timestamps", "block", "episode", "init_pos", "human_action", "agent_action", "prev_x", "prev_y", "next_x", "next_y", "ee_pos_x_prev", "ee_pos_y_prev", "ee_pos_x_next", "ee_pos_y_next", "cmd_acc_human", "cmd_acc_agent")#self.column_names = ("human_action", "agent_action", "ee_pos_x_prev", "ee_pos_y_prev", "ee_pos_x_next", "ee_pos_y_next", "cmd_acc_human", "cmd_acc_agent")
		elif self.num_of_states==6:
			self.column_names = ("timestamps", "block", "episode", "init_pos", "human_action", "agent_action", "prev_x", "prev_y", "next_x", "next_y", "ee_pos_x_prev", "ee_pos_y_prev", "ee_vel_x_prev","ee_vel_y_prev","ee_pos_x_next", "ee_pos_y_next", "ee_vel_x_next","ee_vel_y_next","cmd_acc_human", "cmd_acc_agent","acc_x_prev","acc_y_prev","acc_x_next","acc_y_next","acc_x","acc_y","acc_x_","acc_y_")
		...
		self.unnorm_acc_x_prev=[]
		self.unnorm_acc_x_next=[]
		self.norm_acc_x_prev=[]
		self.norm_acc_y_next=[]
		self.unnorm_acc_y_prev=[]
		self.unnorm_acc_y_next=[]
		self.norm_acc_y_prev=[]
		self.norm_acc_x_next=[]
		#end new
		...
		#self.temp=[("temp","entropy","entropy_loss","q1_history","q2_history","q1_loss","q2_loss","target_q","policy_loss")] #he changed the name to ogu_data
	...
	def reset(self,position):
		...
		try:
			#my code is not included, so ignore it for the time being            
			if(rospy.get_param("/rl_control/Game/gazebo_simulation",False)):
	...
	def run(self, i_episode,block_number,init_position):
		...
		self.start_clock(block_number) #new
		...
		while not rospy.is_shutdown():
			...
			#if not self.test_agent_flag and self.timestep == self.max_timesteps: #he changed the condition
			...
			#self.observation = self.get_state() #changed
			if self.timestep==1:
				vel_x=0
				vel_y=0
				acc_x=0
			...
			self.observation, pos_x, pos_y,vel_x, vel_y = self.get_state(0,0,vel_x,vel_y)

			#new
			tst = datetime.now() #current day and time
			tstamp = int(tst.timestamp() * 1000) #how many secs have passed from current day and time from Jan 1, 1970 UTC to msec.
			#end new
			norm_acc_x=(acc_x-self.min_a_x)/(self.max_a_x-self.min_a_x)
			norm_acc_y=(acc_y-self.min_a_y)/(self.max_a_y-self.min_a_y)
			if norm_acc_x>1:
				norm_acc_x=1
			elif norm_acc_x<0:
				norm_acc_x=0
			...
			if self.num_of_states==6:
				self.observation = np.append(self.observation, [norm_acc_x, norm_acc_y])
			if rospy.get_time() - tmp_time > self.action_duration:
				...
				#randomness_request = self.test_count # this change declares that only the first 10 will be played with R A #removed

				#self.compute_agent_action(randomness_request) #changed
				self.compute_agent_action(block_number)
			
			rospy.sleep(self.action_duration)
			
			#self.observation_ = self.get_state()
			self.observation_, pos_x_, pos_y_,vel_x_,vel_y_  = self.get_state(pos_x,pos_y,vel_x,vel_y)
			acc_x_=(vel_x_-vel_x)/self.action_duration
			acc_y_=(vel_y_-vel_y)/self.action_duration

			norm_acc_x_=(acc_x_-self.min_a_x)/(self.max_a_x-self.min_a_x)
			norm_acc_y_=(acc_y_-self.min_a_y)/(self.max_a_y-self.min_a_y)
			if norm_acc_x_>1:
				norm_acc_x_=1
			elif norm_acc_x_<0:
				norm_acc_x_=0
			...
			if self.num_of_states==6:
				...
				self.observation_=np.append(self.observation_, [norm_acc_x_, norm_acc_y_])
	
			self.reward = self.compute_reward(vel_x_,vel_y_)
			self.episode_reward += self.reward
			#self.done = self.check_if_game_ended() #changed
			self.done = self.check_if_game_ended(block_number,vel_x_,vel_y_)
			
			# for obs in self.observation:
			# 	if obs > 1.2 or obs < -0.2:
			...
			if (self.num_of_states==4)or(self.num_of_states==6):
			...
			if self.num_of_states==6:
				self.norm_acc_x_prev.append(norm_acc_x)
				self.norm_acc_y_prev.append(norm_acc_y)
				self.norm_acc_x_next.append(norm_acc_x_)
				self.norm_acc_y_next.append(norm_acc_y_)
				self.unnorm_acc_x_prev.append(acc_x)
				self.unnorm_acc_x_next.append(acc_x_)
				self.unnorm_acc_y_prev.append(acc_y)
				self.unnorm_acc_y_next.append(acc_y_)
		...
		if self.num_of_states==4:
			...
			print(list(zip(self.timestamps, self.run_blocks, self.run_steps, self.run_init_pos, self.human_actions, self.agent_actions, self.unnorm_pos_x_prev, self.unnorm_pos_y_prev, self.unnorm_pos_x_next, self.unnorm_pos_y_next, self.ee_pos_x_prev, self.ee_pos_y_prev, self.ee_vel_x_prev, self.ee_vel_y_prev, self.ee_pos_x_next, self.ee_pos_y_next, self.ee_vel_x_next, self.ee_vel_y_next, self.cmd_acc_x, self.cmd_acc_y,self.norm_acc_x_prev,self.norm_acc_y_prev,self.norm_acc_x_next,self.norm_acc_y_next,self.unnorm_acc_x_prev,self.unnorm_acc_y_prev,self.unnorm_acc_x_next,self.unnorm_acc_y_next)))
			new_state_info = list(zip(self.timestamps, self.run_blocks, self.run_steps, self.run_init_pos, self.human_actions, self.agent_actions, self.unnorm_pos_x_prev, self.unnorm_pos_y_prev, self.unnorm_pos_x_next, self.unnorm_pos_y_next, self.ee_pos_x_prev, self.ee_pos_y_prev, self.ee_vel_x_prev, self.ee_vel_y_prev, self.ee_pos_x_next, self.ee_pos_y_next, self.ee_vel_x_next, self.ee_vel_y_next, self.cmd_acc_x, self.cmd_acc_y,self.norm_acc_x_prev,self.norm_acc_y_prev,self.norm_acc_x_next,self.norm_acc_y_next,self.unnorm_acc_x_prev,self.unnorm_acc_y_prev,self.unnorm_acc_x_next,self.unnorm_acc_y_next))
		...
		if self.test_agent_flag: #we are in testing state 
			...
			if self.transfer_learning: 
				...
				if not self.load_model_for_training: #if LfD this loops plays, like a no transfer learning, so might change the previous for
					...
					self.save_models = True
	"""
	#the compute_agent_action has changed. Because of the change e_greedy is not needed anymore
	def compute_agent_action(self, block_number):
		self.expert_action_flag = False
		#if you are in test block slect the action with the biggest probability 
		if self.test_agent_flag:
			print("i take argmax")
			self.agent_action = self.agent.actor.greedy_act(self.observation)
		else:
            #if you are in training block and you have transfer learning and you are not in baseline (the 1st block where the player plays with the agent and the agent's actions are completely random)
			if self.transfer_learning and block_number != 1: 
				self.ppr_request = np.random.randint(100)/100
				#sample action based on expert according to ppr_threashold (sample because you are in training)
				if self.ppr_request < self.ppr_threshold:
					self.expert_action_flag = True
					self.agent_action = self.expert_agent.actor.sample_act(self.observation)
				#if you do not pass the threashold sample action from current agent 
				else:
					self.agent_action = self.agent.actor.sample_act(self.observation)
				self.save_models = True
			#if you are in baseline select a random action (for this reason there is sample) 
			else:
				self.agent_action = self.agent.actor.sample_act(self.observation)
				self.save_models = True
	...
	def compute_reward(self,vel_x=0,vel_y=0):
	...
	def check_if_game_ended(self,block_number,vel_x_ee,vel_y_ee):
		pos_x_ee=0
		pos_y_ee=0
		"""
		if(rospy.get_param("/rl_control/Game/gazebo_simulation",False)):
		...
		if (distance.euclidean([pos_x_ee, pos_y_ee], self.goal) <= self.goal_dis and 
			...
			score_msg = Score()
			#score_msg.score.data = self.episode_reward #changed
			score_msg.score.data = 150 + self.episode_reward
			score_msg.block.data = block_number #new
			"""
			if self.timeout:
				...
				#score_msg.outcome.data = True #removed
				score_msg.status.data = "Success" #new
			...
			if self.timeout:
				...
				t_win = threading.Thread(target=play, args=(self.win_audio,))
				t_win.start()
			print("Score Message: ",score_msg) #new
			self.score_pub.publish(score_msg)
			return True
		return False
	...
	def get_state(self,pos_x=0,pos_y=0,vel_x=0,vel_y=0):
		...
		if pos_x_prev==pos_y_prev==0:
			vel_x=vel_x
			vel_y=vel_y

			#print("prev:" ,vel_x,vel_y)
		else:
			vel_x=(pos_x-pos_x_prev)/self.action_duration
			vel_y=(pos_y-pos_y_prev)/self.action_duration 
			if vel_x>self.max_vel_x:
				vel_x=self.max_vel_x
			elif vel_x<self.min_vel_x:
				vel_x=self.min_vel_x
			if vel_y>self.max_vel_y:
				vel_y=self.max_vel_y
			elif vel_y<self.min_vel_y:
				vel_y=self.min_vel_y

			#print("nexv:" ,vel_x,vel_y)


		if (self.num_of_states==4) or (self.num_of_states==6):
		 return np.array([((self.ur3_state.pose.position.x-self.min_x)/(self.max_x-self.min_x)), ((self.ur3_state.pose.position.y-self.min_y)/(self.max_y-self.min_y)), ((vel_x-self.min_vel_x)/(self.max_vel_x-self.min_vel_x)), ((vel_y-self.min_vel_y)/(self.max_vel_y-self.min_vel_y))]), pos_x, pos_y, vel_x, vel_y
		elif (self.num_of_states==2):
		 return np.array([((self.ur3_state.pose.position.x-self.min_x)/(self.max_x-self.min_x)), ((self.ur3_state.pose.position.y-self.min_y)/(self.max_y-self.min_y))]), pos_x, pos_y, vel_x, vel_y
	...
	def grad_updates(self,i_block):
		...
		#self.episode_number += 1 #removed
		return end_grad_updates - start_grad_updates

	def test(self,block_number):
		#change: in the case of 8 games per block then randomely a position but in the same game the same position has to be visited 2 times
		rospy.loginfo("\n ******** Testing ******** \n")
		#the 4 avalilable positions, 2 doubles in order to be visited 2 times
		v = [0,0,1,1,2,2,3,3]
		#shuffle the list in order to add randomeness
		random.shuffle(v)
		#self.test_agent_flag = True #it is changed to what is depicted below:
		
		#test_agent_flag=False in baseline (block=1) because of how compute_agent_action is created, block_number=14 is when there is only human in the game in order to practice in how to control the movement in x axis 
		if block_number == 1 or block_number == 14: # Baseline - sample actions
			if (block_number==1) and (self.train_model==True):
				self.test_agent_flag = False
			elif (block_number==1) and (self.train_model==False):
		...
		for test_i_episode in range(1, self.test_max_episodes+1):
			self.test_count += 1
			position = v[test_i_episode-1] #visit the position from the list
			self.reset(position)
			self.run(test_i_episode,block_number,position)
	...
	"""
	#there are many changes in train function
	#first off all it is broken in 2 seperate functions: train and train_models

	#train_models is responsible for updating the parameters of the network
...
#train decides initial position of the ee in eacch game in training block and runs the game
...
def wait_for_keypress(): 
    ...
    try:
        ...
        curses.endwin()
"""
#it gives to the user the change to finish earlier the experiment by pressin Y or N and also isolates the choise only between Y and N, the other keys are invalid
def wait_for_keypress():
	stdscr = curses.initscr()
	curses.noecho()
	curses.cbreak()
...
def game_loop(game):
	if game.train_model:
		rospy.loginfo('\n ******* Mode: Training ******* \n')
		game.test() 											# test with random agent initial games first 'start_training_on_episode' games
		...
		if  game.lfd_expert_gameplay: #if the expert plays we save all the experience of the gameplay to the expert buffer
			game.agent.memory.save_buffer('/home/kassiotakis/Desktop/catkin_ws5/src/hrc_study_tsitosetal/Paper_Results/buffers/expert_buffer')

	else:
		rospy.loginfo('Testing')
		game.test()
"""
#game loop has also changed significantly but for the better
...
def game_loop(game, data_dir):
	if game.train_model:
		...
		wait_for_keypress()
		#it added a for loop for train inside game loop, if #block_number is devided with 2 you are in training block (which makes sence because each training block is after a test block and you start and finish with a test block 
